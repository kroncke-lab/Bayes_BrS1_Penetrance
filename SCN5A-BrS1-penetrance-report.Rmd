...
setwd("/path/to/GitHub/Bayes_BrS1_Penetrance/")
...
# bcl (structure-based features outputed from biochemical library maintained in the Meiler Lab)
dim(bcl)
# eight duplicates
dim(unique(bcl))
head(bcl)
...
# nonsense variants are clearly important, however, they offer little variability in the covariates
# we use to construct the method and therefore are not included.
...
which is raised at 20th percentile. This aviods having ~3-5 common variants dictate the weighting at the near exclusion of all other variants 
...
## Step 5 Calculate penetrance density from $\tilde\mu_i$ using function `funcdist` from the "func_dist_seq.R" script and "distances" referenced at line 28 above.

Note that only 774 out of 1439 (53.8%) variants fall within structured regions of the protein. For the remaining 46.2% of variants we employ a residue distance estimate of a flexible polymer as described in the supplemental materials.   
...
EM algorithm typically converges in 7 steps.  
...
Scale the variance by a factor of 20. This is subjective and we suggest scaling could be a factor between 10 and 20, equivalent to 9-19 hypothetical observations of clinically phenotyped heterozygous individuals per variant.
...
forest.data <- data.frame(variant = d$var, em.posterior = new_mean, empirical.posterior=alt.mean,
                          lower=lower, higher=higher,total.carriers=d$total_carriers)

forest.data <- forest.data[order(forest.data$variant,forest.data$total.carriers),]

head(forest.data)

plotf <- function(a,b){
  png( paste("Plots/Forest Plots/", a, "-",b,"pics.png",sep=""),res=300,height=10,width=10,units="in")
  forestplot(paste("Variant: ",forest.data[a:b,"variant"],", total carriers:",forest.data$total.carriers[a:b]), 
...
The forests plots are pasted below, where blue boxes are EM posteriors, and red segements are the 95% credible intervals for the EM priors.    
...
The code to produce the weighted correlations is shown in the code chunk. The size of the circles in the presented figures are scaled by the Log<sub>10</sub>(Total Number of Carriers). We additionally calculate 95% confidence intervals on the weighted correlations and AICs.  
