---
title: "SCN5A-LQT3 Penetrance Prediction"
...
d$penetrance_lqt3 <- d$lqt3/d$total_carriers
...
model <- lm(penetrance_lqt3 ~ 1, data=d, weights = d$weight)
summary(model)
...
# Bayesian LQT3 penetrance estimates from empirical priors 
# and observed affected/unaffected counts:
d$lqt3_penetranceBayesian_initial <- (alpha0 + d[,"lqt3"])/((alpha0 + beta0 + d[,"total_carriers"]))
d$lqt3_penetranceBayesian<-d$lqt3_penetranceBayesian_initial
...
d[is.na(d$lqt3_penetranceBayesian),"lqt3_penetranceBayesian"] <- alpha0/(alpha0+beta0)
d$p_mean_w <- d$lqt3_penetranceBayesian
...
p_variance_w <- sapply(index, function(x) pmv(d[x,"lqt3"]+alpha0, d[x,"total_carriers"] - d[x,"lqt3"] + beta0)[,2])
...
for(rec in seq(2,2016,1)){
  #print(rec)
  ld <- funcdist(rec, "var", d[!is.na(d$total_carriers) & d$total_carriers>0 & d$mut_type != "nonsense",], scn5a_dist, "penetrance_lqt3", "sigmoid", 7)
  d[!is.na(d$resnum)& d$resnum == rec,"lqt3_dist"] <- ld[1]
  d[!is.na(d$resnum)& d$resnum == rec,"lqt3_dist_weight"] <- ld[2] 
}
...
covariates <- c("eaRate","blastpssm","provean_score","pph2_prob","ilate","lqt3_dist", "REVEL")
...
d$alpha_1_w <- d$lqt3+alpha0
d$beta_1_w <- d$total_carriers-d$lqt3+beta0
...
d$lqt3_penetrance<-(d$alpha_g+d$lqt3)/(d$total_carriers+d$beta_g+d$alpha_g)
...
d$Structure<-NA
d[!is.na(d$lqt3_dist) & d$lqt3_dist<0.1,"Structure"]<-"Non_Hotspot"
d[!is.na(d$lqt3_dist) & d$lqt3_dist>=0.1 & d$lqt3_dist<0.4,"Structure"]<-"Mild_Hotspot"
d[!is.na(d$lqt3_dist) & d$lqt3_dist>=0.4,"Structure"]<-"Hotspot"

save(d,file="CheckPoint_20_lqt3.RData")
